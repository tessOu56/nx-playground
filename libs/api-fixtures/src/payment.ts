...
export function paymentProviderFromEnv(env: {
  merchantId?: string;
  hashKey?: string;
  hashIV?: string;
}): PaymentProvider {
  const merchantId = env.merchantId?.trim() ?? '';
  const hashKey = env.hashKey?.trim() ?? '';
  const hashIV = env.hashIV?.trim() ?? '';
...
export function mockCheckoutUrl(
  ...
  return `${base}/payments/mock-complete/${intentId}`;
}
...
export function portalOrderReturnUrl(
  ...
  const base = portalPublicUrl.replace(/\/$/, '');
